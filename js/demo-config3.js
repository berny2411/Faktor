...
$(function(){
   ...
   */
   ...
   var idD=document.getElementById("idDoc").value;
   var num=1;
  $('#drag-and-drop-zoneCCP').dmUploader({
    url: 'controller/uploadCE.php',
    multiple:false,
    extraData:{idDo: idD},
...
$(function(){
   ...
   * Las funciones de la interfaz de usuario ui_ * se pueden ubicar en: demo-ui.js
   */
   var idD2=document.getElementById("idDoc").value;
   var num=2;
  $('#drag-and-drop-zoneCFH').dmUploader({
    url: 'controller/uploadCE.php',
    multiple:false,
    extraData:{idDo: idD2},
    maxFileSize: 3000000, // 3 Megs 
...
$(function(){
   ...
   * Las funciones de la interfaz de usuario ui_ * se pueden ubicar en: demo-ui.js
   */
  var idD3=document.getElementById("idDoc").value;
  var num=3;
  $('#drag-and-drop-zoneCFHX').dmUploader({
    url: 'controller/uploadCE.php',
    multiple:false,
    extraData:{idDo: idD3},
    maxFileSize: 3000000,
    allowedTypes: 'text/xml', // 3 Megs 
    auto: false,
    queue: false,
    onDragEnter: function(){
      //Sucede al arrastrar algo sobre el área DnD
  ...
  $('#pills-home-tab4').addClass('nav-link active').attr("aria-selected",'true');
  $('#pills-home4').addClass('nav-link active show');
    },
    onBeforeUpload: function(id){
      // acerca de comenzar a cargar un archivo
      ui_multi_update_file_status(id, 'uploading', 'Uploading...');
...
$(function(){
   ...
   */
   ...
   var idD4=document.getElementById("idDoc").value;
   var numCA=4;
  $('#drag-and-drop-zoneCE').dmUploader({
    url: 'controller/uploadCA.php',
    multiple:false,
    extraData:{idDo: idD4},
    maxFileSize: 3000000, // 3 Megs 
    allowedTypes: 'application/pdf',
    ...
    onNewFile: function(id, file){
       swal('Archivo','En listado','success');
      ui_multi_add_file(id, file,numCA);
    },
    onBeforeUpload: function(id){
  ...
  $('#btnApiStartCA').on('click', function(evt){
    evt.preventDefault();
    

    $('#drag-and-drop-zoneCE').dmUploader('start');
 ...
 $('#files').on('click', 'button.cancel', function(evt){
    ...
    var id4 = $(this).closest('li.media').data('file-id'+4);

    $('#drag-and-drop-zoneCE').dmUploader('cancel', id4);
  ...
  $('#btnApiStart').on('click', function(evt){
    ...
    $('#drag-and-drop-zoneCCP').dmUploader('start');
    $('#drag-and-drop-zoneCFH').dmUploader('start');
    $('#drag-and-drop-zoneCFHX').dmUploader('start');
  });
...
$('#files').on('click', 'button.cancel', function(evt){
    ...
    $('#drag-and-drop-zoneCCP').dmUploader('cancel', id);
    $('#drag-and-drop-zoneCFH').dmUploader('cancel', id2);
    $('#drag-and-drop-zoneCFHX').dmUploader('cancel', id3);
